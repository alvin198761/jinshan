...
public class BMPDecoder {
	...
	private void unpack(byte[] rawData, int rawOffset, int bpp,
	...
	private void unpack24(byte[] rawData, int rawOffset, int[] intData,
			int intOffset, int w) {
		int j = intOffset;
		int k = rawOffset;
		int mask = 0xff;
	...
	private void unpack32(byte[] rawData, int rawOffset, int[] intData,
			int intOffset, int w) {
		int j = intOffset;
		int k = rawOffset;
		int mask = 0xff;
	...
	public static Image getBMPImage(String imagePath) throws Exception {
		InputStream is = null;
		try {
			is = new BufferedInputStream(new FileInputStream(
					new File(imagePath)));
			return getBMPImage(is);
		} catch (Exception e) {
			throw e;
		}
	}

	public static Image getBMPImage(InputStream is) throws Exception {
	...
	public static ImageIcon getBMPIcon(InputStream is) throws Exception {
		return new ImageIcon(getBMPImage(is));
	}

	public static ImageIcon getBMPIcon(String imagePath) throws Exception {
		return new ImageIcon(getBMPImage(imagePath));
	}

	public static void main(String[] aqgs) throws Exception {
		JFrame jf = new JFrame();
		JLabel jl = new JLabel();
		ImageIcon ii = getBMPIcon("D:\\tangzc\\java\\eclipsework\\threadDemo\\src\\test\\Test.bmp");
		jl.setIcon(ii);
		jf.add(jl);
		jf.pack();
		jf.setVisible(true);
